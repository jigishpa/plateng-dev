(function () {
  ...
  // Rendered from data/adrs.json, which is generated by
  // multicloud-platform-ai/scripts/sync-adrs.py from the canonical ADRs.
  // This list was previously hardcoded here and drifted: five of ten summaries
  // described decisions those numbers do not refer to. Do not hand-edit it.
  function renderADRs(data) {
    var container = document.getElementById("adr-list");
    if (!container || !data.adrs || !data.adrs.length) return;

    var html = "";
    for (var i = 0; i < data.adrs.length; i++) {
      var a = data.adrs[i];
      html +=
        "<li>" +
  ...
  loadJSON("data/adrs.json", renderADRs);
})();
